...
export default function Header({ 
  ...
  const filteredProviders = React.useMemo(() => {
    ...
    const filtered = !q
      ? providers
      : providers
          .map((p) => ({ ...p, models: p.models.filter((m) => m.toLowerCase().includes(q)) }))
          .filter((p) => p.models.length > 0);
    console.log(`[Header] searchQuery="${searchQuery}" providers=${providers.length} filteredProviders=${filtered.length}`);
    return filtered;
      ...
      <div ref={menuRef} style={{ position: 'relative' }}>
        ...
        >
          ...
          <span style={{ fontSize: 14, fontWeight: 600 }}>
            {currentProvider ? `${currentProvider.name}: ${activeModel}` : activeModel}
          </span>
        ...
        {modelMenuOpen && (
          ...
          >
            {providers.length > 0 ? (
              <>
                <div style={{ padding: '4px 8px 8px', flexShrink: 0 }}>
                  <input
                    ...
                    placeholder={t('chat_model_search_placeholder')}
                    style={{
                      width: '100%',
                  ...
                  {filteredProviders.length > 0 ? (
                    filteredProviders.map((p) => (
                      <div key={p.id}>
                        ...
                        {p.models.map((model) => {
                          const isActive = activeProvider === p.id && activeModel === model;
                          return (
                            <div
                              key={`${p.id}-${model}`}
                              onClick={() => {
              ...
              <div style={{ padding: '10px', fontSize: 12, color: 'var(--color-text-400)', textAlign: 'center' }}>
                <div>No AI providers configured.</div>
                <button
